...
export default function Navbar() {
  ...
  const mobileMenuVariants = {
    hidden: { scaleY: 0, opacity: 0, transformOrigin: "top", transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] } },
    visible: { scaleY: 1, opacity: 1, transformOrigin: "top", transition: { duration: 0.7, ease: [0.22, 1, 0.36, 1] } },
  };

  const mobileItemVariants = {
    hidden: { x: -50, opacity: 0 },
    visible: { x: 0, opacity: 1, transition: { delay: 0.3, type: "spring", stiffness: 150, damping: 15 } },
  };
  ...
  return (
    <>
      ...
      <nav className="w-full fixed top-0 z-50 flex items-center justify-between px-6 py-4 md:px-12 bg-white/60 backdrop-blur-md border-b border-gray-100">
        <motion.h1 className="text-2xl md:text-2xl font-extrabold text-indigo-900 tracking-tight cursor-pointer" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
        ...
        {/* Mobile Hamburger */}
        <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden focus:outline-none p-2 rounded-full hover:bg-gray-100 transition-colors duration-200" aria-label="Toggle Mobile Menu">
          <motion.svg className="w-8 h-8 text-indigo-900" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24" animate={{ rotate: menuOpen ? 90 : 0 }} transition={{ duration: 0.3 }}>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            ...
            >
              <svg className="w-8 h-8 text-indigo-700" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
